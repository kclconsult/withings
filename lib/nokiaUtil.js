...
const NOKIA_CALLBACK = querystring.escape(config.CALLBACK_BASE + "/nokia/connect/callback");
...
class NokiaUtil {
  	...
  	static genURLFromRequestToken(req, res, base_url, callback, params = {}) {
  	    ...
  	    if ( config.OAUTH_VERSION == 2 ) {
            ...
            util.postRequest(config.NOKIA_ACCESS_TOKEN_BASE_OAUTH2, bodyData, function(error, response, body) {

                body = JSON.parse(body);

                if ( body.access_token && body.refresh_token ) {

                  // ~MDC Ugly. Remove all this.
                  ...
                  callback("Refreshing tokens failed.");
    ...
    static translateNokiaData(data) {
